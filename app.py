...
st.set_page_config(page_title="World Cup Portfolio Kelly", layout="wide")
    ...
    for fixture in config.REMAINING_FIXTURES:
        ...
        if h2h_table.empty:
            ...
            fair_df = pd.DataFrame(
                {
                    "Outcome": list(fair_probs.keys()),
                    "Consensus fair probability": [f"{p:.1%}" for p in fair_probs.values()],
                    "Best available odds": [f"{best_odds.get(k, float('nan')):.2f}" for k in fair_probs.keys()],
                }
            )
            st.markdown("**Consensus fair probability (de-vigged, averaged across bookmakers)**")
...
with tab3:
    ...
    if not bets:
        ...
        display_df["Edge"] = display_df["edge_pct"].map(lambda x: f"{x:+.2f}%")
        display_df["Fair prob"] = display_df["fair_prob"].map(lambda x: f"{x:.1%}")
        display_df["Market implied prob"] = display_df["market_implied_prob"].map(lambda x: f"{x:.1%}")
        display_df["Recommended stake %"] = display_df["recommended_stake_pct"].map(lambda x: f"{x:.2f}%")
        display_df["Recommended stake amount"] = display_df["recommended_stake_amount"].map(lambda x: f"{x:,.2f}")
        display_df["Status"] = display_df["no_bet"].map(lambda x: "🚫 No bet" if x else "✅ Bet")

        st.markdown("**Combined portfolio -- every candidate bet with edge and recommended stake**")
        st.dataframe(
            display_df[
                [
                    "label", "best_odds", "Fair prob", "Market implied prob",
                    "Edge", "Recommended stake %", "Recommended stake amount", "Status",
                ]
            ].rename(columns={"label": "Bet", "best_odds": "Best odds"}),
        ...
        if chart_df.empty:
            ...
            fig = px.bar(
                chart_df.sort_values("recommended_stake_pct"),
